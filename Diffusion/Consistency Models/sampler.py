...
class DiffusionSampler:
    ...
    def get_eps(self, x:torch.Tensor, t:torch.Tensor, c:torch.Tensor, *, uncond_scale:float, uncond_cond:Optionla[torch.Tensor]):

        return self.model(x,t,c)

    def sample(self, shape: List[int], cond:torch.Tensor, repeat_noise: bool=False, temperature:float=1., x_last:Optional[torch.Tensor]=None, uncond_sclae: float=1., uncond_cond: Optional[torch.Tensor]=None, skip_steps: int=0):
