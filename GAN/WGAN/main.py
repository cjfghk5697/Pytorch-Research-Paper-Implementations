...
print(device)
dl=get_data(params['batch_size'],params['num_workers'],params['img_size'],params['pin_memory'])
...
fixed_noise=Variable(Tensor(np.random.normal(0, 1, (data.shape[0], params['nz']))))
...
for epoch in range(1,params['epoch']+1):
    for i,(data,_) in enumerate(dl):
        ...
        noise=Variable(Tensor(np.random.normal(0, 1, (data.shape[0], params['nz']))))
    ...
    if((epoch+1) == 1 or (epoch+1) == params['epoch']/2) or epoch%10==0:
        ...
        plt.savefig("Epoch_%d {}".format('Generate') %(epoch))
        plt.close('all') 
    iters += 1